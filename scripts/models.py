...
def build_results_markdown(payload: dict) -> str:
    """Render the markdown block that lives between the README markers."""
    split = payload["split"]
    ...
    lines.append("")
    lines.append(
        "_Auto-generated by `scripts/models.py` "
        f"(grid search took {payload['elapsed_grid_search_min']} min)._"
    )
    lines.append("")
    lines.append("### Split sizes")
    lines.append("")
    lines.append("| Split | Rows | Positive rate |")
    lines.append("| ----- | ----:| -------------:|")
    lines.append(
        f"| Train | {split['train']} | {split['train_pos_rate']:.3f} |"
    )
    lines.append(
        f"| Val   | {split['val']} | {split['val_pos_rate']:.3f} |"
    )
    lines.append(
        f"| Test  | {split['test']} | {split['test_pos_rate']:.3f} |"
    )
    lines.append(
        f"| Kaggle submission (unlabeled) | {split['kaggle_submission']} | — |"
    )
    lines.append("")

    lines.append("### Best hyperparameters (selected by val F1)")
    lines.append("")
    lines.append("| Model | Best config | Val F1 |")
    lines.append("| ----- | ----------- | ------:|")
    for name in ["LSTM", "BERTweet", "RoBERTa"]:
        lines.append(
            f"| {name} | {_fmt_config(best_cfg[name])} | {best_val[name]:.4f} |"
        )
    lines.append("")

    lines.append("### Best ensemble (selected by val F1)")
    lines.append("")
    w = ens["weights"]
    lines.append("| LSTM weight | BERTweet weight | RoBERTa weight | Threshold | Val F1 |")
    lines.append("| ----------: | --------------: | -------------: | --------: | ------:|")
    lines.append(
    ...
    for name in ["LSTM", "BERTweet", "RoBERTa"]:
        lines.append(
            f"| {name} | {test_pm[name]['test_f1']:.4f} | "
            f"{test_pm[name]['test_acc']:.4f} |"
        )
    lines.append(
        f"| **Ensemble** (val-tuned weights & threshold) | "
        f"**{ens['test_f1']:.4f}** | **{ens['test_acc']:.4f}** |"
    )
    lines.append("")
    lines.append(
        "_The test split was held out from training, grid search, "
        "ensemble-weight search and threshold tuning._"
    )
    lines.append("")

    lines.append("### Performance curves (best config from each grid search)")
    lines.append("")
    lines.append("| Train F1 | Val F1 |")
    lines.append("| -------- | ------ |")
    lines.append(
        "| ![Train F1](plots/performance/train_f1.png) | "
        "![Val F1](plots/performance/val_f1.png) |"
    )
    lines.append("")
    lines.append("| Train Loss | Val Loss |")
    lines.append("| ---------- | -------- |")
    lines.append(
        "| ![Train Loss](plots/performance/train_loss.png) | "
        "![Val Loss](plots/performance/val_loss.png) |"
    )
    lines.append("")
    lines.append("| Train Accuracy | Val Accuracy |")
    lines.append("| -------------- | ------------ |")
    lines.append(
        "| ![Train Acc](plots/performance/train_acc.png) | "
        "![Val Acc](plots/performance/val_acc.png) |"
    )
    lines.append("")

    lines.append("### Exploratory data analysis")
    lines.append("")
    lines.append("| Label distribution | Word count by class |")
    lines.append("| ------------------ | ------------------- |")
    lines.append(
        "| ![Label pie](plots/eda/tweet_label_piChart.png) | "
        "![Word count](plots/eda/tweet_word_count_histogram.png) |"
    )
    lines.append("")
    lines.append("| Mean word length | Missing values |")
    lines.append("| ---------------- | -------------- |")
    lines.append(
        "| ![Word length](plots/eda/tweet_word_count_histogram_pdf.png) | "
        "![Missing values](plots/eda/missing_values.png) |"
    )
    lines.append("")
    lines.append("![Top keywords by class](plots/eda/top_keywords_by_class.png)")
    lines.append("")
